...
internal sealed class PostgresFifoInboxStorageProvider : PostgresInboxStorageProviderBase, ISupportGroupLocksReleaseStorageProvider
{
    ...
    public PostgresFifoInboxStorageProvider(
        IInboxConfiguration configuration,
        IProviderOptionsAccessor optionsAccessor,
        ILogger<PostgresFifoInboxStorageProvider> logger)
    ...
    public async Task ReleaseMessagesAndGroupLocksAsync(IReadOnlyList<IInboxMessageIdentifiers> messages, CancellationToken token)
    {
        ...
        await RetryExecutor.ExecuteAsync(async ct =>
        {
            ...
            await combinedCmd.ExecuteNonQueryAsync(ct);
        }, token);
    }
        ...
        CancellationToken token)
    {
        ...
        return await RetryExecutor.ExecuteAsync(async ct =>
        {
            ...
            var result = await combinedCmd.ExecuteScalarAsync(ct);
    ...
    public override async Task MigrateAsync(CancellationToken token)
    {
        await RetryExecutor.ExecuteAsync(async ct =>
        {
            ...
            try
            {
                ...
                await groupLocksCmd.ExecuteNonQueryAsync(ct);
