...
[Collection("Postgres")]
public class PostgresProcessingTests(PostgresContainerFixture container, ITestOutputHelper output) : IAsyncLifetime
{
    ...
    public async Task EndToEnd_WriteAndProcess_AllMessagesProcessed()
    {
        ...
        _serviceProvider = CreateServiceProvider("e2e-basic", handler);
        ...
        await writer.WriteBatchAsync(messages, "e2e-basic");
    ...
    public async Task EndToEnd_BatchedHandler_ProcessesInBatches()
    {
        ...
        _serviceProvider = CreateBatchedServiceProvider("e2e-batched", handler);
        ...
        var messages = TestMessageFactory.CreateSimpleMessages(messageCount);
        await writer.WriteBatchAsync(messages, "e2e-batched");
    ...
    {
        var handler = new BatchedCountingHandler<SimpleMessage>();
        _serviceProvider = CreateBatchedServiceProvider("e2e-throughput", handler, o =>
        {
            o.ReadBatchSize = 200;
        ...
        {
            await writer.WriteBatchAsync(batch, "e2e-throughput");
        }
        writeSw.Stop();
