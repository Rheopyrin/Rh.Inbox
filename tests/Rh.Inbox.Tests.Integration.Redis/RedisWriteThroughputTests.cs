...
[Collection("Redis")]
public class RedisWriteThroughputTests(RedisContainerFixture container, ITestOutputHelper output) : IAsyncLifetime
{
    ...
    public async Task WriteAsync_SingleMessages_MeasuresThroughput()
    {
        ...
        _serviceProvider = CreateServiceProvider("redis-write-single", handler);
        ...
        {
            await writer.WriteAsync(new SimpleMessage($"msg-{i}", $"data-{i}"), "redis-write-single");
        }
        sw.Stop();

        var messagesPerSecond = messageCount / sw.Elapsed.TotalSeconds;
        output.WriteLine($"Single write: {messagesPerSecond:F2} msg/s ({sw.ElapsedMilliseconds}ms for {messageCount} messages)");
        messagesPerSecond.Should().BeGreaterThan(500);
    }
    ...
    {
        var handler = new CountingHandler<SimpleMessage>();
        _serviceProvider = CreateServiceProvider("redis-write-batch", handler);
    ...
    {
        var handler = new CountingHandler<SimpleMessage>();
        _serviceProvider = CreateServiceProvider("redis-write-concurrent", handler);
        ...
        var tasks = Enumerable.Range(0, writerCount).Select(async writerId =>
        {
            ...
            {
                await writer.WriteBatchAsync(batch, "redis-write-concurrent");
            }
        });
        ...
        messagesPerSecond.Should().BeGreaterThan(10000);
    }
